...
{
    internal record AssemblyMetadata(string Name, string Version, string Culture, string PublicKeyToken);
    ...
    public class FrameworkAssemblyLocator : Microsoft.Build.Utilities.Task, IMultiThreadableTask
    {
        ...
        private AssemblyMetadata? GetAssemblyInfo(string assemblyPath)
        {
            try
            {
        ...
        private void CollectResults(Dictionary<string, string?> resolutions)
        {
            ...
            foreach (var kvp in resolutions)
            {
                if (kvp.Value != null)
                {
                    ...
                    AssemblyMetadata? metadata = GetAssemblyInfo(kvp.Value);
                    if (metadata != null)
                    {
